	...
	orderEvent: thunk(async (actions, payload, { getStoreState, dispatch }) => {
		...
		await fetch(server + 'order/event/', {
			...
			.catch((code) => {
				switch (code) {
					case 400:
						alert('Ошибка формирования заказа: скорее всего ваш предыдущий заказ не оплачен. Попробуйте перейти по предыдущей ссылке оплаты в истории браузера (ctrl + H / cmd + Y) или оплатить еще раз через 10 минут.')
						break;
					case 500:
						alert('Сервер не отвечает')
						break;
